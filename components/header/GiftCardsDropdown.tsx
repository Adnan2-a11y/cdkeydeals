...
const DEFAULT_GIFT_CARDS: NavItem[] = [
...
export default function GiftCardsDropdown({
  isOpen,
  onToggle,
  onClose,
  dynamicItems,
}: GiftCardsDropdownProps) {
  const dropdownRef = useRef<HTMLDivElement>(null);
  const timeoutRef  = useRef<NodeJS.Timeout | null>(null);

  const items = dynamicItems && dynamicItems.length > 0 ? dynamicItems : DEFAULT_GIFT_CARDS;
